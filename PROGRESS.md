...
**3. 制約違反サマリーパネル**
- **左ペインに表示**: 違反がある場合のみ表示される折りたたみ可能なパネル
...
- `src/hooks/useConstraintCheck.ts` - 制約チェックロジックを実装したカスタムフック
- `src/components/SortableTimetableRow.tsx` - 行レベルの違反表示（背景色、アイコン、ツールチップ）
- `src/components/CoolSection.tsx` - クール内の違反をフィルタして各行に渡す
- `src/components/TimetableDropZone.tsx` - フラット構造の違反表示対応
- `src/components/TimetableEditing.tsx` - 制約違反サマリーパネルの実装
